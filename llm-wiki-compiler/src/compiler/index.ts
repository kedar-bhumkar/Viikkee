...
async function runCompilePipeline(root: string): Promise<CompileResult> {
  ...
  const result = summarizeCompile(buckets, generation, extractions);
  await safelyAppendLog(root, generation.pages, result);
  return result;
}
...
/**
 * Append a compile log entry without failing compilation.
 * Logging is non-critical — disk errors should warn but never abort a build.
 */
async function safelyAppendLog(
  root: string,
  pages: MergedConcept[],
  result: CompileResult,
): Promise<void> {
  const newConcepts = pages.filter((p) => p.concept.is_new).map((p) => p.slug);
  const updatedConcepts = pages.filter((p) => !p.concept.is_new).map((p) => p.slug);
  ...
  try {
    await appendCompileLog(root, entry);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    output.status("!", output.warn(`Skipped compile log: ${message}`));
  }
}
