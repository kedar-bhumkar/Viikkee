...
/**
 * Find concept pages that contain a Contradictions section.
 * These pages flag factual conflicts detected during compilation and warrant
 * human review before being treated as authoritative.
 */
export async function checkContradictions(root: string): Promise<LintResult[]> {
  const pages = await readMarkdownFiles(path.join(root, CONCEPTS_DIR));
  const results: LintResult[] = [];

  for (const page of pages) {
    const lines = page.content.split("\n");
    const contradictionLineIndex = lines.findIndex((l) =>
      /^##\s+⚠️\s+Contradictions/i.test(l),
    );
    if (contradictionLineIndex !== -1) {
      results.push({
        rule: "contradiction-detected",
        severity: "warning",
        file: page.filePath,
        message: "Page contains a Contradictions section — review conflicting claims",
        line: contradictionLineIndex + 1,
      });
    }
  }
